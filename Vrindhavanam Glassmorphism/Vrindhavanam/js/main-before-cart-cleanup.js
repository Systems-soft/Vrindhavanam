...
function addToCart(name, price, emoji, selectId) {
    ...
    if (selectId) {
        ...
        if (select) {
            ...
            const weight = selectedText.split('â€”')[0].trim();
            finalName = `${name} (${weight})`;
        }
    }
    const numPrice = parseCurrencyValue(price);
...
function addToCartFromAPI(productName) {
    ...
    console.log("Clicked:", productName); // âœ… ADD
    ...
    console.log("Found product:", product); // âœ… ADD
...
function updateCartUI() {
    ...
    const total = cart.reduce((a, i) => a + (Number(i.price) || 0) * i.qty, 0);
    totalEl.textContent = 'â‚¹' + total.toLocaleString('en-IN');
    if (cart.length === 0) {
        itemsEl.innerHTML = '<div class="cart-empty">ðŸ›’<p>Your cart is empty.<br>Add some spices!</p></div>';
        ...
        <div class="cart-item">
          ...
          <div class="cart-item-info">
            ...
            <div class="cart-item-meta">â‚¹${(Number(item.price) || 0).toLocaleString('en-IN')} each</div>
            <div class="cart-qty">
          ...
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:0.5rem">
            <div class="cart-item-price">â‚¹${((Number(item.price) || 0) * item.qty).toLocaleString('en-IN')}</div>
            <button class="cart-remove" onclick="removeFromCart('${item.name}')">âœ•</button>
          </div>
        </div>
      `).join('');
    }
...
function traceSpice() {
    ...
    if (match) {
        ...
        if (verifiedBadge) {
            ...
            verifiedBadge.textContent = "âœ“ Certified Organic";
        }
        ...
        showToast(`Traced: ${match.product_name} (${match.variety_name}) â€” Verified âœ…`);
    } else {
        ...
        traceResult.classList.add('show');
        showToast(`Trace Failed: Batch "${batch}" Not Verified âš ï¸`);
    }
}
...
console.log("CARD FOUND");
    ...
    option.textContent =
        `${weightText} â€” â‚¹${roundedPrice.toLocaleString("en-IN")}`;
});
...
select.addEventListener("change", function () {
    ...
    if (varietyPrice) {
        varietyPrice.textContent =
            "â‚¹" + Number(this.value).toLocaleString("en-IN");
    }
    ...
    if (mainPrice) {
        ...
        mainPrice.textContent =
            selectedOption.text.split("â€”")[0].trim();
    }
});
        ...
        if (isHomepageSelect) {
            ...
            if (varietyPrice) {
                varietyPrice.textContent = "â‚¹" + Number(select.value).toLocaleString("en-IN");
            }
            const mainPrice = card.querySelector(".product-price");
            if (mainPrice) {
                const selectedOption = select.options[select.selectedIndex];
                mainPrice.textContent = selectedOption ? selectedOption.text.split('â€”')[0].trim() : '';
            }
        }
    }
...
const selectedWeight = Number(select.value);
    ...
    priceEl.textContent =
        "â‚¹" + Math.round(calculatedPrice).toLocaleString("en-IN");
}
// TOAST
let toastTimer;
